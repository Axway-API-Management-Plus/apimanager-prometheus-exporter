...
async function createRegistry(pluginConfig, logger) {
    if(pluginConfig.registry) {
        ...
        registryMetric( client.Gauge, { name: 'axway_apigateway_version_info',               help: 'Version information about the API-Gateway. Value indicates if service if running or not.', labelNames: ['gatewayId', 'version', 'image'], registers: []});
        ...
        registryMetric( client.Histogram, { name: 'axway_api_requests_duration_seconds', help: 'The API-Request duration. (ANM metric: processingTimeAvg)', buckets: [0.01, 0.02, 0.05, 0.1, 0.25, 0.5, 1], labelNames: ['gatewayId', 'service'], registers: []});
